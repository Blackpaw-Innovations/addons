    ...
    def create_mrp_from_pos(self, products):
        ...
        if products:
            ...
            for prod in product_ids:
                if prod['qty'] > 0:
                    # Check available stock before creating manufacturing order
                    product = self.env['product.product'].browse(prod['id'])
                    location_id = prod.get('location_id')
                    available_qty = product.with_context(location=location_id).qty_available
                    missing_qty = prod['qty'] - available_qty
                    if missing_qty <= 0:
                        # Sufficient stock, skip manufacturing order creation
                        continue
                    ...
                    if bom_count:
                        ...
                        if bom:
                            vals = {
                                ...
                                'product_qty': missing_qty,  # Manufacture only missing qty
                                'bom_id': bom.id,
                            }
