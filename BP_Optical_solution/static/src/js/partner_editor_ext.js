...
import { InsurancePopup } from "@BP_Optical_solution/js/insurance_popup";
...
patch(PartnerDetailsEdit.prototype, {
    ...
    async loadOpticalBranches() {
        try {
            const branches = await this.env.services.orm.searchRead(
                "optical.branch",
                [["active", "=", true]],
                ["id", "name"],
                { order: "name" }
            );
            this.opticalBranches = branches;
        } catch (error) {
            console.error("Failed to load optical branches:", error);
            this.opticalBranches = [];
        }
    },
