...
class AccountMove(models.Model):
    ...
    def _generate_pdf_and_send_invoice(self, template=False, **kwargs):
        """Override to handle PDF generation errors gracefully for POS orders."""
        try:
            return super()._generate_pdf_and_send_invoice(template=template, **kwargs)
        except Exception as e:
            # Log the error but don't fail the invoice creation
            _logger.warning(
                "Failed to generate/send invoice PDF for invoice %s: %s. "
                "Invoice was created successfully but PDF generation was skipped.",
                self.name, str(e)
            )
            # Return empty wizard action to avoid breaking the flow
