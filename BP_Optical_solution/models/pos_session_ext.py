...
class PosSession(models.Model):
    ...
    def _loader_params_optical_branch(self):
        """Load optical branches for POS"""
        return {
            'search_params': {
                'domain': [('active', '=', True)],
                'fields': ['id', 'name'],
    ...
    def _get_pos_ui_optical_branch(self, params):
        """Load branches for POS UI"""
        return self.env['optical.branch'].search_read(**params['search_params'])
    ...
    def _pos_data_process(self, loaded_data):
        """Override to add insurance companies and branches to loaded data"""
        ...
        if self.config_id.optical_enabled:
            ...
            loaded_data['optical.branch'] = self._get_pos_ui_optical_branch(
                self._loader_params_optical_branch()
            )
