    ...
    def optical_create_test(self, order_uid, partner_id, test_vals):
        ...
        Args:
            ...
            test_vals: Dictionary containing all test fields:
                - Current RX: sphere_od, cylinder_od, axis_od, prism_od, add_od, va_od, pd_od, height_od
                - Current RX: sphere_os, cylinder_os, axis_os, prism_os, add_os, va_os, pd_os, height_os
                - Previous RX: prev_sphere_od, prev_cylinder_od, prev_axis_od, prev_prism_od, prev_add_od, prev_va_od, prev_pd_od, prev_height_od
                - Previous RX: prev_sphere_os, prev_cylinder_os, prev_axis_os, prev_prism_os, prev_add_os, prev_va_os, prev_pd_os, prev_height_os
                - Patient info: phone_number, age, branch_id, optician_id
                - Lens & Frame: lens_type_id, coating_id, index_id, material_id, frame_id, needs_new_lens, needs_new_frame
                - Symptoms: symptom_ids (list of IDs)
                - Other: notes, valid_until, workshop_order_number, follow_up_required, follow_up_date
        ...
        try:
            ...
            # Get branch from test_vals or POS config
            branch_id = test_vals.get('branch_id', False)
            if not branch_id and order_uid:
            ...
            # Add optician if provided
            if test_vals.get('optician_id'):
                vals['optician_id'] = test_vals['optician_id']
            
            # Add phone_number and age if provided
            if test_vals.get('phone_number'):
                vals['phone_number'] = test_vals['phone_number']
            if test_vals.get('age') is not False:
                vals['age'] = test_vals['age']
            ...
            if test_vals.get('height_od') is not False:
                vals['height_od'] = test_vals['height_od']
            ...
            if test_vals.get('add_os') is not False:
                vals['add_os'] = test_vals['add_os']
            if test_vals.get('va_os'):
                vals['va_os'] = test_vals['va_os']
            if test_vals.get('pd_os') is not False:
                vals['pd_os'] = test_vals['pd_os']
            if test_vals.get('height_os') is not False:
                vals['height_os'] = test_vals['height_os']
            
            # Map Previous RX - OD (Right Eye) fields
            if test_vals.get('prev_sphere_od') is not False:
                vals['prev_sphere_od'] = test_vals['prev_sphere_od']
            if test_vals.get('prev_cylinder_od') is not False:
                vals['prev_cylinder_od'] = test_vals['prev_cylinder_od']
            if test_vals.get('prev_axis_od') is not False:
                vals['prev_axis_od'] = test_vals['prev_axis_od']
            if test_vals.get('prev_prism_od') is not False:
                vals['prev_prism_od'] = test_vals['prev_prism_od']
            if test_vals.get('prev_add_od') is not False:
                vals['prev_add_od'] = test_vals['prev_add_od']
            if test_vals.get('prev_va_od'):
                vals['prev_va_od'] = test_vals['prev_va_od']
            if test_vals.get('prev_pd_od') is not False:
                vals['prev_pd_od'] = test_vals['prev_pd_od']
            if test_vals.get('prev_height_od') is not False:
                vals['prev_height_od'] = test_vals['prev_height_od']
            
            # Map Previous RX - OS (Left Eye) fields
            if test_vals.get('prev_sphere_os') is not False:
                vals['prev_sphere_os'] = test_vals['prev_sphere_os']
            if test_vals.get('prev_cylinder_os') is not False:
                vals['prev_cylinder_os'] = test_vals['prev_cylinder_os']
            if test_vals.get('prev_axis_os') is not False:
                vals['prev_axis_os'] = test_vals['prev_axis_os']
            if test_vals.get('prev_prism_os') is not False:
                vals['prev_prism_os'] = test_vals['prev_prism_os']
            if test_vals.get('prev_add_os') is not False:
                vals['prev_add_os'] = test_vals['prev_add_os']
            if test_vals.get('prev_va_os'):
                vals['prev_va_os'] = test_vals['prev_va_os']
            if test_vals.get('prev_pd_os') is not False:
                vals['prev_pd_os'] = test_vals['prev_pd_os']
            if test_vals.get('prev_height_os') is not False:
                vals['prev_height_os'] = test_vals['prev_height_os']
            
            # Map Lens & Frame fields
            if test_vals.get('lens_type_id'):
                vals['lens_type_id'] = test_vals['lens_type_id']
            if test_vals.get('coating_id'):
                vals['coating_id'] = test_vals['coating_id']
            if test_vals.get('index_id'):
                vals['index_id'] = test_vals['index_id']
            if test_vals.get('material_id'):
                vals['material_id'] = test_vals['material_id']
            if test_vals.get('frame_id'):
                vals['frame_id'] = test_vals['frame_id']
            if test_vals.get('needs_new_lens') is not None:
                vals['needs_new_lens'] = test_vals['needs_new_lens']
            if test_vals.get('needs_new_frame') is not None:
                vals['needs_new_frame'] = test_vals['needs_new_frame']
            
            # Map Symptoms (Many2many field)
            if test_vals.get('symptom_ids'):
                symptom_ids = test_vals['symptom_ids']
                if isinstance(symptom_ids, list) and symptom_ids:
                    vals['symptom_ids'] = [(6, 0, symptom_ids)]
            
            # Map additional fields
            if test_vals.get('notes'):
                vals['notes'] = test_vals['notes']
            if test_vals.get('workshop_order_number'):
                vals['workshop_order_number'] = test_vals['workshop_order_number']
            if test_vals.get('follow_up_required') is not None:
                vals['follow_up_required'] = test_vals['follow_up_required']
            if test_vals.get('follow_up_date'):
                vals['follow_up_date'] = test_vals['follow_up_date']
