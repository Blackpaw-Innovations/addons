...
class ResPartnerPosExt(models.Model):
    ...
    patient_branch_id = fields.Many2one(
        'optical.branch',
        string='Patient Branch',
        compute='_compute_patient_branch',
        store=False,
    )

    @api.depends()
    def _compute_patient_branch(self):
        """Get branch from optical.patient record if exists"""
        for partner in self:
            patient = self.env['optical.patient'].search([
                ('partner_id', '=', partner.id)
            ], limit=1)
            partner.patient_branch_id = patient.branch_id if patient else False
    ...
    def create_from_ui(self, partner):
        ...
        if not is_optical_pos:
            # Not optical POS - clean optical fields and use standard creation
            optical_fields = [
                'has_insurance', 'insurance_company_id', 'policy_number',
                'insurance_expiry_date', 'patient_company', 'insurance_invoice_number',
                'coverage_details', 'document', 'document_name', 'insuranceData',
        ...
        # Extract optical insurance fields and other non-partner fields
        ...
        has_insurance = partner.pop('has_insurance', False)
        insuranceData = partner.pop('insuranceData', None)
        document = partner.pop('document', None)
        document_name = partner.pop('document_name', None)
        ...
        # Get gender from partner data
        gender = partner.get('gender', False)
        
        # Get branch_id from partner data, or fallback to POS config branch
        branch_id = partner.pop('branch_id', None)
        if not branch_id and pos_session.config_id.optical_branch_id:
            branch_id = pos_session.config_id.optical_branch_id.id
        ...
        patient_vals = {
            ...
            'gender': gender if gender else False,
            'branch_id': branch_id,
        ...
        if not patient_vals['branch_id']:
            ...
            if default_branch:
                ...
                # Clean optical fields before fallback
                optical_fields = [
                    'has_insurance', 'insurance_company_id', 'policy_number',
                    'insurance_expiry_date', 'patient_company', 'insurance_invoice_number',
                    'coverage_details', 'document', 'document_name', 'insuranceData'
        ...
        try:
            ...
            # Fallback to standard partner creation - clean optical fields first
            optical_fields = [
                'has_insurance', 'insurance_company_id', 'policy_number',
                'insurance_expiry_date', 'patient_company', 'insurance_invoice_number',
                'coverage_details', 'document', 'document_name', 'insuranceData'
