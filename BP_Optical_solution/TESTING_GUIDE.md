...
**Module**: BP_Optical_solution v17.0.1.0.0  
**Date**: November 2025  
...
- ✅ `BP_Optical_solution` (installed and configured)
- ✅ `BP_Optical_solution` (this module)

### Database Setup
```bash
# Ensure module is installed on correct database
sudo -u odoo /opt/odoo/odoo-bin -c /etc/odoo.conf -d [YOUR_DB] -u BP_Optical_solution --stop-after-init
...
| **Optical Branch** | Select branch from BP_Optical_solution | ✓ Yes |
...
- ✅ Insurance company exists in BP_Optical_solution
...
sudo -u odoo /opt/odoo/odoo-bin -c /etc/odoo.conf -d [YOUR_DATABASE] -u BP_Optical_solution --stop-after-init
sudo systemctl restart odoo
...
2. Verify module status: Navigate to Apps → BP_Optical_solution
3. Check dependencies installed: BP_Optical_solution, point_of_sale, account
